  ...
  return (
    <section className="relative w-full max-w-6xl mx-auto pb-16 md:pb-8">
        ...
        <div className="flex gap-0">
          {filteredSlides.length === 0 && searchTerm ? (
            ...
            filteredSlides.map((s, idx) => (
              <div
                key={idx}
                className="flex-none w-full sm:w-1/2 md:w-1/2 lg:w-1/3 px-2"
              >
                ...
                >
                  <article className="relative overflow-hidden rounded-md shadow-lg h-full">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-end pb-6 md:pb-4 pointer-events-none">
                      <p
                        className={`text-black text-3xl font-bold py-0 ${dmSerif.className}`}
                      >
                        {s.title}
                      </p>
      ...
      <button
        onClick={scrollPrev}
        aria-label="Anterior"
        className="absolute -left-2 top-1/2 -translate-y-1/2 rounded-full bg-white p-2 text-black drop-shadow z-30 hover:bg-[#399BEB]  hidden md:block"
      >
        <ChevronLeft size={14} />
      </button>
      <button
        onClick={scrollNext}
        aria-label="Próximo"
        className="absolute -right-2 top-1/2 -translate-y-1/2 rounded-full bg-white p-2 text-black drop-shadow z-30 hover:bg-[#399BEB] hidden md:block"
      >
